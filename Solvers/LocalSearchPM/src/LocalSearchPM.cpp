...
LocalSearchPM::LocalSearchPM(LocalSearchPM& orig) :IterativeAlg(orig), intRNG(NULL), floatRNG(NULL), obj(NULL)  {
	...
	this->intRNG = orig.intRNG->clone();
	this->floatRNG = orig.floatRNG->clone();
...
void LocalSearchPM::setRandGens(Common::Interfaces::RandGen<Math::uint32>* irg, Common::Interfaces::RandGen<double>* frg) {
	...
	if (irg == NULL) {
		Debugger::err << "LocalSearchPM::setRandGen : Trying to clone a NULL random number generator!!!" << ENDL;
	}

	this->intRNG = irg->clone();
...
//	objElapsedMS += objTimer.elapsed();
	...
	//if (!debugCheckPMCorrectness("LocalSearchPM::assessActions")) {
