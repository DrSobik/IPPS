...
void VNSPlanner::init() {
    ...
    //schedSolvers.clear(); // Causes the algorithm to crash when ran again: schedSolvers are not recreated
    ...
    out << "VNSPlanner::init : Running initScheduler..." << endl;

    //  #####################  Run the init scheduler ######################

    QFuture<Schedule> initSchedFuture = QtConcurrent::run(initScheduler.getPointer(), &SchedSolver::solve, curSP);
    curSched = initSchedFuture.result();
    if (initSchedFuture.isFinished()) {
	out << "VNSPlanner::init : Finished init scheduler!" << endl;
    }

    //  ####################################################################

    out << "VNSPlanner::init : Done running initScheduler." << endl;
    ...
    out << "#########################################################" << endl;
    out << "VNSPlanner::init : bestObj = " << bestSched.objective << endl;
    out << "#########################################################" << endl;
